...
D_sys = D;
D_sys_mod = zeros(7,2);
sys_sys = ss(A_sys,B_sys,C_sys_mod,D_sys_mod);
%% Kalman Gain
v_vel = .1e-6;
v_trq = 35e-3;
w_trq = .1e-3;
w_vel = .15e-3;
Qk = diag([ 0, 10e-14 , 1.225e-3, 0, 1.225e-3, 0, 0]);
...
%For the control system to reject ramp disturbnaces
%a double integrators is required
%This is a basic result in control engineering

%Appending a double integrator to design LQR
Aiii = [ A_sys(1:6 , 1:6)  , zeros(6,2);
        -Ct(: , 1:6 )     , zeros(1,2);
         0 0 0 0 0 0 1 0];
...
[K_lq3,S_lq3,P_lq3] = lqr(Aiii,Biii,Qiii,R);
