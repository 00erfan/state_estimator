...
eqns = [ Tm - bm*tetad_act - J_act*TETADD_ACT - (Kg/rg)*( (teta_act/rg) - teta_int ) - (dgs/rg)*( (tetad_act/rg) - tetad_int ) - (dg/rg)*(tetad_act/rg) == 0 ,
         Kg*( (teta_act/rg) - teta_int ) + dgs*( (tetad_act/rg) - tetad_int ) - Js_int*TETADD_INT == 0 ];
...
vars_mat = [ TETAD_ACT TETADD_ACT TETAD_INT TETADD_INT ];

[TETAD_ACT TETADD_ACT TETAD_INT TETADD_INT] = solve(eqns_mat, vars_mat)
...
eqns_mat_form = [ TETAD_ACT TETADD_ACT TETAD_INT TETADD_INT ];
vars_mat_form = [ teta_act tetad_act teta_int tetad_int ];
...
% R = .5; % A number greater than zero
